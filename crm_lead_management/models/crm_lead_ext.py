...
class CrmLead(models.Model):
    ...
    inactive_archive_date = fields.Date(
        string="Ngày archive inactive",
        readonly=True,
        copy=False,
        tracking=True,
    )
    archived_by_cron = fields.Boolean(
        string="Archive tự động",
        readonly=True,
        copy=False,
    )
            ...
            if exact_duplicates:
                raise UserError(
                    _(
                        ...
                        "Vui lòng kiểm tra lại hoặc yêu cầu Sales Manager phê duyệt xử lý dữ liệu trùng."
                    )
    ...
    def create(self, vals_list):
        default_stage = self.env.ref("thuc_linh_crm_lead_management.stage_new_lead", raise_if_not_found=False)
    ...
    def unlink(self):
        if not self.env.user.has_group("sales_team.group_sale_manager") and not self.env.context.get("crm_delete_approval_id"):
            raise UserError(_("Xóa lead phải được Sales Manager phê duyệt trước."))
        return super().unlink()
