...
class CrmLostReasonAnalytics(models.Model):
    ...
    _description = 'Theo dõi Lost Deal'
    _order = 'lost_date desc'

    lead_id = fields.Many2one('crm.lead', string='Lead')
    lead_name = fields.Char(string='Tên Lead')
    lost_reason_id = fields.Many2one('crm.lost.reason', string='Lý do Lost')
    user_id = fields.Many2one('res.users', string='Nhân viên Sales')
    expected_revenue = fields.Float(string='Doanh thu kỳ vọng')
    lost_date = fields.Datetime(string='Ngày Lost')
