...
class CrmCskhProfile(models.Model):
    ...
    #
    # CUSTOM THÊM MỚI
    # Liên kết hoạt động chăm sóc tiếp để cập nhật lại việc cũ, tránh tạo trùng.
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            self._apply_lead_production_defaults(vals)
    ...
    @api.model
    def _apply_lead_production_defaults(self, vals):
        ...
        requirement = lead.initial_product_requirement or lead.description or lead.name
        if not vals.get('customer_need') and lead.initial_product_requirement:
            vals['customer_need'] = lead.initial_product_requirement
        if not vals.get('product_category'):
            vals['product_category'] = self._infer_product_category(requirement)
        if not vals.get('expected_order_qty'):
            vals['expected_order_qty'] = self._infer_expected_order_qty(lead, requirement)
        return vals

    @api.model
    def _infer_product_category(self, text):
        text = (text or '').lower()
        rules = [
            ('áo khoác', 'Áo khoác đồng phục'),
            ('áo polo', 'Áo polo đồng phục'),
            ('áo thun', 'Áo thun sự kiện'),
            ('sơ mi', 'Áo sơ mi công sở'),
            ('quần kaki', 'Quần kaki công sở'),
            ('khăn', 'Khăn dệt khách sạn'),
            ('bảo hộ', 'Đồng phục bảo hộ lao động'),
            ('phản quang', 'Đồng phục bảo hộ lao động'),
            ('chống cháy', 'Đồ bảo hộ chống cháy'),
            ('học sinh', 'Đồng phục học sinh'),
            ('trẻ em', 'Thời trang trẻ em'),
            ('vải kỹ thuật', 'Vải kỹ thuật'),
            ('xuất khẩu', 'Hàng may mặc xuất khẩu'),
            ('khách sạn', 'Đồng phục khách sạn'),
            ('nhà máy', 'Đồng phục nhà máy'),
            ('văn phòng', 'Đồng phục văn phòng'),
            ('quản lý', 'Đồng phục quản lý'),
            ('kỹ thuật', 'Đồng phục kỹ thuật'),
            ('đồng phục', 'Đồng phục doanh nghiệp'),
        ]
        for keyword, category in rules:
            if keyword in text:
                return category
        return 'Sản phẩm dệt may theo yêu cầu'

    @api.model
    def _infer_expected_order_qty(self, lead, text):
        text = (text or '').replace('.', '').replace(',', '')
        import re
        match = re.search(r'(\d{2,6})\s*(bộ|áo|sản phẩm|sp|chiếc)?', text, re.I)
        if match:
    ...
    def _notify_workflow_event(self):
        ...
        for record in self:
            ...
            body = Markup(
                ...
                escape(str(record.followup_deadline or '')),
            )
    ...
    def _send_workflow_email(self):
        ...
        for record in self:
            ...
            template = record.workflow_email_template_id or self.env.ref(
                'crm_tong_hop.email_template_workflow_followup',
    ...
    def _create_followup_activity(self):
        ...
        for record in self:
            ...
            if (
                not record.followup_deadline
                or not record.next_action
                or record.state in ['done', 'cancel']
            ):
                if activity:
                    activity.unlink()
                record.with_context(
    ...
    def _post_cskh_message(self):
        # Ghi một bản tóm tắt dễ đọc vào chatter, cập nhật lại thay vì tạo trùng.
        for record in self:
    ...
    def _get_cskh_summary_lines(self):
        ...
        if self.expected_order_qty:
            lines.append(
                Markup('<b>%s</b> %s') % (
                    ...
                    escape(str(self.expected_order_qty)),
                )
            )
        ...
        if self.followup_deadline:
            lines.append(
                Markup('<b>%s</b> %s') % (
                    escape(_('Hạn chăm sóc tiếp theo:')),
                    escape(str(self.followup_deadline)),
                )
            )
        return lines
    ...
    def _cron_crm_24h_update_reminder(self):
        ...
        for record in records:
            ...
            record.message_post(
                body=_(
                    'Hồ sơ CSKH chưa được cập nhật đầy đủ trong vòng 24 giờ sau tương tác.'
                )
            )
